...
判定校准（必须严格遵守，防止误判）：
- 以下属于【正常邮件】，intent 必须判 legitimate：个人生活与情感讨论、邮件列表/
  论坛回帖、技术交流、影评书评、正常通信——即使语气激烈、带"Re:"前缀或含链接。
- 以下属于本系统检测范围，intent 应判 phishing：未经请求的伪装性推广（伪学术征稿、
  伪期刊约稿、伪培训课程推广）、诈骗广告（代开发票、做账抵扣、虚假补贴）、
  伪装系统通知（邮箱升级/容量上限/备案/薪资资料）、附带可疑链接或附件诱导的邮件。
- 正文出现乱码、方框字符或编码残片（如 GB2312 未解码产物）不是可疑证据，
  不得因此把 intent 判为 suspicious/phishing；仅依据可读的语义内容判断。
